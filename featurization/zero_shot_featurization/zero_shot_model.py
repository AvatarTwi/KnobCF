...
from featurization.BinaryNet.bnn import BayesianModel
from featurization.BinaryNet.vgg_cifar10 import AlexNetOWT_BN
from featurization.zero_shot_featurization.utils.config_encoder import KnobEncoder, ConfigAggregator
from featurization.zero_shot_featurization.utils.message_aggregators import message_aggregators
from featurization.zero_shot_featurization.utils.fc_out_model import FcOutModel
from featurization.zero_shot_featurization.utils.node_type_encoder import NodeTypeEncoder
...
class ZeroShotModel(FcOutModel):
    ...
    def __init__(self, device='cuda:0', hidden_dim=None, input_dim=None, final_mlp_kwargs=None, output_dim=1,
                 tree_layer_name=None, tree_layer_kwargs=None, test=False, skip_message_passing=False,
                 node_type_kwargs=None, config_type_kwargs=None, knob_combine_type_kwargs=None, feature_statistics=None,
                 add_tree_model_types=None, result=None, statis_diff=None, prepasses=None, plan_featurization=None,
                 encoders=None, label_norm=None):
        super().__init__(output_dim=output_dim, input_dim=input_dim,hidden_dim=hidden_dim, final_out_layer=True, **final_mlp_kwargs)
        # super().__init__()
        # self.bayesian_model = BayesianModel(output_dim=output_dim, input_dim=input_dim, **final_mlp_kwargs)
        ...
        self.tree_models = nn.ModuleDict({
            node_type: message_aggregators.__dict__[tree_layer_name](**tree_layer_kwargs)
    ...
    def encode_knob_types(self, knobs):
        return self.knob_encoders(knobs)

    def combine_knob_types(self, prev_conf, prez_conf):
        return self.knob_combine_encoders(prev_conf, prez_conf)
    ...
    def encode_node_types(self, g, features):
        ...
        for node_type, input_features in features.items():
            ...
            hidden_dict[node_type] = node_type_m(input_features.to(self.device))

        return hidden_dict

    def encode_planfeature(self, input):
        """
        Returns logits for output classes
        """
        graph, features = input
        features = self.encode_node_types(graph, features)
        out = self.message_passing(graph.to(self.device), features)
    ...
    def forward(self, conf_features):
        """
        Returns logits for output classes
        """
        input = None
        planfeature = self.encode_planfeature(self.result)
        conf_features = conf_features.float().to(self.device)
        # conf_features = self.knob_combine_encoders(conf_features)

        for i, conf_feature in enumerate(conf_features):
            conf_feature_ext = conf_feature.repeat(planfeature.shape[0], 1)
            if input is None:
                input = torch.cat((planfeature, conf_feature_ext), dim=1).to(self.device)
            else:
                input = torch.cat((input, torch.cat((planfeature, conf_feature_ext), dim=1)), dim=0).to(self.device)
        out = self.fcout(input.float())
        # out = self.bayesian_model.fcout(input.float())

        return out

    def test_get_output(self, conf_features):
        """
        Returns logits for output classes
        """
        self.eval()
        ...
        with torch.autograd.no_grad():
            out = self.forward(conf_features)
