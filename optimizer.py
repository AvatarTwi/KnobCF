...
from optimizer_model.bestconf import BEST_CONFIG
...
def get_ddpg_optimizer(config, spaces, tae_runner, state):
    ...
    # DDPG Model
    from optimizer_model.ddpg import DDPG, DDPGOptimizer
...
def get_new_hyperspace(input_space):
    new_input_space = ConfigSpace.ConfigurationSpace()
    for hyperparameter in input_space.get_hyperparameters():
        if isinstance(hyperparameter, ConfigSpace.CategoricalHyperparameter):
            # 删除原有的离散值超参数，添加一个一样的浮点超参数
            print(hyperparameter.default_value)
            float_hyperparam = ConfigSpace.UniformFloatHyperparameter(name=hyperparameter.name + '_CHANGED', lower=0.0,
                                                             upper=1.999999,
                                                             default_value=1.0 if hyperparameter.default_value == 'on' else 0)
            new_input_space.add_hyperparameter(float_hyperparam)

        else:
            new_input_space.add_hyperparameter(hyperparameter)
    print(input_space)
    print(new_input_space)
    logger.info(f'\ninput space:\n{new_input_space}')
    return new_input_space

def get_bestconf_optimizer(config, spaces, tae_runner, state):
    logger.info('Constructing new optimizer...')

    # Generate input (i.e. knobs) and output (i.e. perf metric) space
    input_space = spaces.generate_input_space(
        config.seed, ignore_extra_knobs=None)     # 返回一个参数空间

    input_space = get_new_hyperspace(input_space)
    ...
    scenario = Scenario({
        "run_obj": "quality",
        "runcount-limit": config.iters, # 限制运行的次数
        "cs": input_space,
        "deterministic": "true",
        "always_race_default": "false",
        # disables pynisher, which allows for shared state
        "limit_resources": "false",
        "output_dir": state.results_path, # outdir=results
    })
    ...
    initial_design = LHDesignWithBiasedSampling(**init_design_def_kwargs)
    tae_runner = partial(tae_runner, state=state)
    optimizer = BEST_CONFIG(state, tae_runner, initial_design, config.iters, logging_dir=state.results_path)
    return optimizer