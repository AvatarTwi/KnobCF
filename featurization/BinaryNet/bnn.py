...
class BayesianModel(BSequential):
    """Bayesian neural network with dropout."""

    def __init__(self, input_dim, output_dim, n_layers=None, width_factor=None, task=None, nonlin=torch.nn.LeakyReLU,
                 output_nonlin=None, weight_initializer=partial(
                torch.nn.init.xavier_normal_,
                gain=torch.nn.init.calculate_gain("leaky_relu", 0.2)), bias_initializer=partial(
                torch.nn.init.uniform_, a=-1.0, b=1.0), dropout_layers=CDropout, input_dropout=None, *args, **kwargs):
        ...
        dims = [self.input_dim] + [int(width_factor * self.input_dim)] * n_layers

        if not isinstance(dropout_layers, Iterable):
            dropout_layers = [dropout_layers] * n_layers
        ...
        # Output layer.
        modules["fc_out"] = torch.nn.Linear(dims[-1], output_dim)
        ...
        self.fcout = BSequential(modules)
        self.fcout.apply(init)


def bayesian_model(input_dim,
                   output_dim,
                   hidden_dim,
    ...
    if not isinstance(dropout_layers, Iterable):
        dropout_layers = [dropout_layers] * len(hidden_dim)
