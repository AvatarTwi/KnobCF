...
class ZeroShotSingle(QueryEncoder):
    def __init__(self, args, c_pool=None):
        super().__init__(args, c_pool=None)
        self.c_pool = c_pool
        logger.addHandler(
            logging.FileHandler(args.model_save_path +
                                '/query_encoder_log/' + args.mode.split('_')[0] + '_KnobCF_' + self.args.workload
                                + '.log', mode='w'))
        self.args = args
        self.workload = self.args.workload
        self.benchmarks = [self.args.workload]
        self.valid_range = {}
        self.valid_queries_each = {}
        pass_keys_all = []
        sum = 0
        # for benchmark in self.benchmarks:
        pass_keys, _, _ = Statistic.data_statistics_queries(workload=self.args.workload)
        temp = sum - len(pass_keys)
        pass_keys_all.extend([i + sum for i in pass_keys])
        sum += queries_num[self.args.workload]
        self.valid_range[self.args.workload] = (temp, sum - len(pass_keys_all))
        self.valid_queries_each[self.args.workload] = [i for i in range(queries_num[self.args.workload]) if i not in pass_keys]
        self.valid_queries = [i for i in range(sum) if i not in pass_keys_all]

    def parse_benchmark(self):
        parsed_runs = self.parse_run([self.args.workload])
        parsed_runs['parsed_plans'] = list(np.array(parsed_runs['parsed_plans'])[self.valid_queries])
        self.result, self.feature_statistics = dataset_creation(parsed_runs)
        labels = []
        c_pool = ConfigurationPool(self.args, self.args.workload)
        c_pool.collect_data()
    ...
    def save_model(self, model_name=''):
        ...
        with open(os.path.join(self.save_dir, 'query_encoder', 'query_encoder_zeroshotsingle_' + model_name + '.pickle'), 'wb') as file_model:
            pickle.dump(self.query_encoder, file_model)

    def load_model(self, model_name=''):
        with open(os.path.join(self.save_dir, 'query_encoder', 'query_encoder_zeroshotsingle_' + model_name + '.pickle'), 'rb') as f:
