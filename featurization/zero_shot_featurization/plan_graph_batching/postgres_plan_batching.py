...
from featurization.benchmark_tools.generate_workload import Operator
from featurization.zero_shot_featurization import postgres_plan_featurizations
from featurization.zero_shot_featurization.utils.feature_statistics import FeatureType
...
def plan_to_graph(node, plan_depths, plan_features, plan_to_plan_edges, db_statistics, feature_statistics,
    ...
    plan_params = copy.copy(node["plan_parameters"])
    ...
    if output_columns is not None:
        for output_column in output_columns:
            output_column_node_id = output_column_idx.get(
                (output_column['aggregation'], tuple(output_column['columns'])))

            # if not, create
            if output_column_node_id is None:
                curr_output_column_features = [encode(column, copy.copy(output_column), feature_statistics)
                ...
                output_column_idx[(output_column['aggregation'], tuple(output_column['columns']))] \
                    = output_column_node_id

                # featurize product of columns if there are any
                db_column_features = db_statistics['column_stats']
                for column in output_column['columns']:
                    column_node_id = column_idx.get(column)
                    if column_node_id is None:
                        curr_column_features = [
                            encode(feature_name, copy.copy(db_column_features[column]), feature_statistics)
                        ...
                        column_idx[column] = column_node_id
    ...
    if table is not None:
        table_node_id = table_idx.get(table)
        db_table_statistics = db_statistics['table_stats']

        if table_node_id is None:
            curr_table_features = [encode(feature_name, copy.copy(db_table_statistics[table]), feature_statistics)
...
def parse_predicates(db_column_features, feature_statistics, filter_column, filter_to_plan_edges, plan_featurization,
    ...
    :return:
    """
    if not isinstance(filter_column, dict):
        return
    ...
    if filter_column['operator'] in {str(op) for op in list(Operator)}:
        curr_filter_features = [encode(feature_name, copy.copy(filter_column), feature_statistics)
                                for feature_name in plan_featurization.FILTER_FEATURES]

        if filter_column['column'] is not None:
            curr_filter_col_feats = [
                encode(column, copy.copy(db_column_features[filter_column['column']]), feature_statistics)
